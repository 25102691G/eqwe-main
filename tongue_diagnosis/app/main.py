"""FastAPI entry point for the standalone tongue segmentation service."""
...
SERVICE_NAME = "tongue_diagnosis_fastapi"
...
def create_app(segmenter: TongueSegmenter | None = None) -> FastAPI:
    """Create the FastAPI application with one configurable segmenter instance."""

    app = FastAPI(
        title="Tongue Diagnosis Service",
        version="0.1.0",
        description="Standalone FastAPI service for tongue segmentation.",
    )

    model_path = Path(os.getenv("TONGUE_MODEL_PATH", str(default_model_path())))
    output_dir = Path(os.getenv("TONGUE_OUTPUT_DIR", str(default_output_dir())))
    app.state.segmenter = segmenter or TongueSegmenter(model_path=model_path, output_dir=output_dir)

    @app.get("/health", response_model=HealthResponse)
    def health() -> HealthResponse:
        current_segmenter: TongueSegmenter = app.state.segmenter
        return HealthResponse(
            status="healthy" if current_segmenter.is_model_available() else "unhealthy",
            service=SERVICE_NAME,
            model_loaded=current_segmenter.is_model_available(),
            model_path=str(current_segmenter.model_path),
        )

    @app.post("/api/v1/tongue/segment", response_model=SegmentResponse)
    async def segment_tongue(
        ...
        try:
            artifacts = current_segmenter.segment_image(image)
        ...
        if include_visualizations and artifacts.cropped_image is not None and artifacts.region_masks is not None:
        ...
        if save_outputs:
            for item in current_segmenter.save_outputs(
                file_stem=Path(file.filename).stem,
                artifacts=artifacts,
            ):
                saved_files.append(SavedFile(**item))

        segmented_image_base64 = None
        mask_image_base64 = None
        if include_images:
            segmented_image_base64 = current_segmenter.encode_image_to_base64(artifacts.segmented_image)
            mask_image_base64 = current_segmenter.encode_image_to_base64(artifacts.mask_image)

        return SegmentResponse(
            status="success",
            message="tongue segmentation complete",
